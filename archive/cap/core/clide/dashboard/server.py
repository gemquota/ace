...
class SubsystemStreamManager:
    ...
    async def route_event(self, message: Dict[str, Any]):
        """Routes generic cognitive events to specific subsystem streams."""
        if message.get("type") != "COGNITIVE_EVENT":
            return
        ...
        if layer == "CLIDE":
            await self.broadcast_to_subsystem("CLIDE", message)
        elif layer == "APC":
            await self.broadcast_to_subsystem("APC", message)
        elif layer == "PXE":
            # PIE consumes PXE layer events (Inference)
            await self.broadcast_to_subsystem("PIE", message)
